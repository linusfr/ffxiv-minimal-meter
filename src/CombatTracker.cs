...
public sealed class CombatTracker : IDisposable
{
    ...
    // and "OtherDamage = 11" was actually MpGain. Verified against
    // FFXIVClientStructs, ravahn/FFXIV_ACT_Plugin and perchbirdd/DamageInfoPlugin,
    // which agree on the byte layout.
    ...
    // ── ActorControl hook (the only place DoT ticks are visible) ──────────────
    //
    // DoT ticks never pass through ActionEffectHandler.Receive. They arrive as
    // ActorControl packets, category 0x17 (HoT_DoT in Machina's enum, which is
    // what ACT keys on). FFXIVClientStructs exposes the client-side handler, so
    // this needs no packet capture — same shape of hook as the two above.
    //
    // The packet names the target and the amount but NOT the source, so ticks
    ...
    /// <summary>
    /// Guards session state (Combatants dictionaries, Store.TempSessions) against
    /// StatusApi's HttpListener thread, which enumerates both while the game
    /// thread is inserting into them. Uncontended, so the hot path pays ~nothing.
    /// </summary>
    public static readonly object StateGate = new();
    ...
    private long _targetInitDamage;
    private bool _targetInitCrit;
    private bool _targetInitDh;
        ...
        string               configDir)
    {
        ...
        unsafe
        {
            ...
            {
                _log.Warning($"MinimalMeter: UseAction hook failed — {ex.Message}");
            }

            try
            {
                var acAddr = FFXIVClientStructs.FFXIV.Client.Network.PacketDispatcher
                                               .Addresses.HandleActorControlPacket.Value;
                _actorControlHook = gameInterop.HookFromAddress<ActorControlDelegate>(
                    (nint)acAddr, OnActorControl);
                _actorControlHook.Enable();
            }
            catch (Exception ex)
            {
                // Non-fatal: without it we simply keep the old blind spot.
                _log.Warning($"MinimalMeter: ActorControl hook failed — {ex.Message}");
            }
        }
        ...
        _log.Info("MinimalMeter: CombatTracker initialized.");
    }
    ...
    private void OnTerritoryChanged(uint newTerritoryId)
    {
        ...
        catch (Exception ex) { _log.Error($"MinimalMeter: Instance summary error — {ex.Message}"); }
    ...
    private void TryCreateInstanceSummary()
    {
        ...
        lock (StateGate) { Store.TempSessions.Add(summary); }
        PruneTempSessions();
        SaveStore();
        _log.Info($"MinimalMeter: Instance summary — {summary.Id} ({pulls.Count} pulls)");
    }
        ...
        foreach (var pull in pulls)
        {
            ...
            foreach (var (entityId, src) in pull.Combatants)
            {
                if (!summary.Combatants.TryGetValue(entityId, out var dst))
                {
                    ...
                    lock (StateGate) { summary.Combatants[entityId] = dst; }
                }
    ...
    private void StartSession()
    {
        ...
        catch (Exception ex) { _log.Warning($"MinimalMeter: CombatLog start failed — {ex.Message}"); }

        _log.Info($"MinimalMeter: Combat started — {ActiveSession.Id}");
    ...
    private void EndSession()
    {
        ...
        {
            lock (StateGate) { Store.TempSessions.Add(ActiveSession); }
            PruneTempSessions();
            SaveStore();
        }
        ...
        _log.Info($"MinimalMeter: Combat ended — {ActiveSession.Id} ({ActiveSession.FormattedDuration})");
    ...
    private void PruneTempSessions()
    {
        while (Store.TempSessions.Count > _config.MaxTempHistory)
            lock (StateGate) { Store.TempSessions.RemoveAt(0); }
    }
        ...
        bool*                                               outOptAreaTargeted)
    {
        ...
        catch (Exception ex) { _log.Error($"MinimalMeter: UseAction original failed — {ex.Message}"); }
        ...
        catch (Exception ex) { _log.Error($"MinimalMeter: UseAction log failed — {ex.Message}"); }
    ...
    private unsafe void OnReceiveActionEffect(
        ...
        GameObjectId*                      targetEntityIds)
    {
        // NOTE: the initial-hit snapshot registers are reset per TARGET inside
        // ProcessEffects, not here. Resetting once per packet meant that in a
        // multi-target ActionEffect, target N's ApplyStatusEffectTarget entry
        // read whatever damage target N-1 rolled, so the DoT simulator took its
        // baseline from the wrong hit.
        ...
        {
            _log.Error($"MinimalMeter: Hook error — {ex.Message}");
        }
        finally
        {
        ...
        GameObjectId*                      targetEntityIds)
    {
        ...
        for (int t = 0; t < numTargets && t < 32; t++)
        {
            ...
            // Per-target snapshot: a Damage entry for THIS target refills these,
            // and this target's ApplyStatusEffectTarget entry reads them.
            _targetInitDamage = 0;
            _targetInitCrit   = false;
            _targetInitDh     = false;
            ...
            for (int e = 0; e < EffectsPerTarget; e++)
            {
                ...
                switch (kind)
                {
                    ...
                    case EffectKind.ParriedDamage:
                    {
                        ...
                        {
                            _targetInitDamage = value;
                            _targetInitCrit   = isCritical;
                            _targetInitDh     = isDirectHit;
                    ...
                    {
                        // Feed the attribution map first, for every caster. This
                            ...
                            && DoTSimulator.IsSimulatorRelevant(actionId))
                        {
                            _dotSim.OnApply(
                                ...
                                casterId:        localId,
                                actionId:        actionId,
                                initialHitValue: _targetInitDamage,
                                initialCrit:     _targetInitCrit,
                                initialDh:       _targetInitDh,
    ...
    public void SaveSession(CombatSession session)
    {
        ...
        lock (StateGate) { Store.TempSessions.Remove(session); }
        SaveStore();
    }
    ...
    private void LoadStore()
    {
        try
        {
            if (File.Exists(_storePath))
            {
                ...
                _log.Info($"MinimalMeter: Loaded {Store.TempSessions.Count} temp + {Store.SavedSessions.Count} saved sessions.");
            }
        }
        catch (Exception ex)
        {
            _log.Error($"MinimalMeter: Failed to load sessions — {ex.Message}");
    ...
    public void SaveStore()
    {
        ...
        {
            _log.Error($"MinimalMeter: Failed to save sessions — {ex.Message}");
        }
    }
    ...
    //
    // FlyText events do NOT carry source / target entity IDs, and the client only
    // draws the local player's numbers anyway, so this path can only ever credit
    // the local player. Other players' DoTs are handled instead by the
    // ActorControl hook above — see docs/DOT_ATTRIBUTION.md.
    //
    // Healing FlyText kinds (Healing, HealingCrit) cover both direct heals and
    // HoT ticks — the icon trick works the same way.
    // ── ActorControl: the only timely source of DoT ticks ────────────────────
        ...
        GameObjectId targetId, bool isRecorded)
    {
        ...
        {
            _log.Debug($"MinimalMeter: ActorControl handler error — {ex.Message}");
        }
        finally
        {
            // Always pass the packet on, whatever happened above.
            _actorControlHook!.Original(entityId, category, arg1, arg2, arg3, arg4,
    ...
    private void HandlePeriodicTick(uint targetEntityId, uint arg1, uint arg2, uint arg3, uint arg4)
    {
        ...
        if (isHeal)
        {
            source.TotalHealingDone += amount;
            source.HealingEvents.Add((tickMs, amount));
            RecordAbility(source.HealingByAbility, HotPseudoActionId, "Heal over Time", amount);
        }
        else
        {
            source.TotalDamageDealt += amount;
            source.DamageEvents.Add((tickMs, amount));
            RecordAbility(source.DamageByAbility, DotPseudoActionId, "Damage over Time", amount);
        }
        ...
        ref bool handled)
    {
        try
        {
            ...
            unsafe
            {
                ...
                if (isDamageTick)
                {
                    // CREDITING DISABLED. FlyText for damage
                ...
                {
                    // CREDITING DISABLED, for exactly the reason the damage branch
                    // above is disabled: healing FlyText can lag its ActionEffect
                    // by 4-6s while DedupWindowMs is 2000, so a late popup escapes
                    // the dedup buffer and gets counted a SECOND time. Worse, the
                    // client also draws healing it did not originate (another
                    // healer's HoT ticking on you), which this path would book as
                    // local healing — inflating the local player's HPS.
                    //
                    // Real HoT ticks arrive on the ActorControl hook alongside
                    // DoTs (category 0x17); crediting them belongs there, where
        ...
        {
            _log.Error($"MinimalMeter: FlyText hook error — {ex.Message}");
        }
    }
    ...
    private CombatantData GetOrCreateLimitBreakCombatant(CombatSession session)
    {
        ...
        lock (StateGate) { session.Combatants[LimitBreakEntityId] = data; }
        return data;
    }
    ...
    //
    // DoT capture is moving off FlyText because its popups are queued visually
    // and can lag their ActionEffect by 4-6 seconds, which made value-based dedup
    // unreliable. The
    ...
    private void OnLogMessage(Dalamud.Game.Chat.ILogMessage msg)
    {
        ...
        catch (Exception ex) { _log.Error($"MinimalMeter: LogMessage capture failed — {ex.Message}"); }
    }
    ...
    private void OnChatMessage(Dalamud.Game.Chat.IHandleableChatMessage chat)
    {
        ...
        catch (Exception ex) { _log.Error($"MinimalMeter: Chat log capture failed — {ex.Message}"); }
    }
    ...
    public void Dispose()
    {
        ...
        _combatLog.Dispose();
        _log.Info("MinimalMeter: CombatTracker disposed.");
    }
}
