...
/// <summary>
/// Exposes MinimalMeter live data to other plugins via Dalamud IPC.
/// Consumed by MinimalMeterUmbra to avoid a direct cross-context DLL reference.
///
/// Providers:
///   "MinimalMeter.GetActiveSummary"  → Func&lt;string&gt;  JSON summary of the active session
///   "MinimalMeter.GetPartyMarkers"   → Func&lt;string&gt;  JSON array of party member combat data
/// </summary>
public sealed class IpcBridge : IDisposable
{
    ...
    public IpcBridge(Plugin plugin)
    {
        ...
        _providerActiveSummary = Plugin.PluginInterface.GetIpcProvider<string>("MinimalMeter.GetActiveSummary");
        _providerActiveSummary.RegisterFunc(GetActiveSummary);

        _providerPartyMarkers = Plugin.PluginInterface.GetIpcProvider<string>("MinimalMeter.GetPartyMarkers");
        _providerPartyMarkers.RegisterFunc(GetPartyMarkers);

        Plugin.Log.Info("MinimalMeter: IPC bridge initialized.");
    }

    // ── MinimalMeter.GetActiveSummary ──────────────────────────────────────────
    //
    // Returns JSON:
    //   { "isActive": false }
    // or:
    ...
    private string GetActiveSummary()
    {
        ...
        {
            Plugin.Log.Error($"MinimalMeter IPC GetActiveSummary error: {ex.Message}");
            return """{"isActive":false}""";
        }
    }

    // ── MinimalMeter.GetPartyMarkers ───────────────────────────────────────────
    //
    ...
    private string GetPartyMarkers()
    {
        ...
        {
            Plugin.Log.Error($"MinimalMeter IPC GetPartyMarkers error: {ex.Message}");
            return "[]";
        }
    }
    ...
    public void Dispose()
    {
        ...
        Plugin.Log.Info("MinimalMeter: IPC bridge disposed.");
    }
}
